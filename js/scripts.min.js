    ...
    if (modifyMode) {
        $.ajax({
            ...
            success: (res) => {
                ...
                handleMainImage(newTopbar.at(-1), '#img-sport-left');
                handleMainImage(newTopbar.at(1), '#img-sport-right');
            },
            error: (e) => {
                console.log(e);
            }
...
async function loadMetadata(genderSport) {
    ...
    if (topbar) {
        handleMainImage(topbar.at(-1), '#img-sport-left');
        handleMainImage(topbar.at(1), '#img-sport-right');
    } else {
        $.ajax({
            url: location.href.split('?')[0] + 'topbar.json',
            type: 'GET',
            success: (res) => {
                handleMainImage(res.at(-1), '#img-sport-left');
                handleMainImage(res.at(1), '#img-sport-right');
                localStorage.setItem('topbar', JSON.stringify(res));
            },
            error: (e) => {
                console.log(e);
